	...
	public:
		...
		BOOL Open(const std::string& Url, const std::string& method)
		{
			...
			if (WinHttpCheckPlatform())
			{
				...
				textconv_helper::A2W_ tempUrl(Url.c_str());
				std::wstring wstrUrl((LPCWSTR)tempUrl);

				if (!WinHttpCrackUrl(wstrUrl.c_str(), wstrUrl.length(), 0, &urlComp))
				{
				...
				textconv_helper::A2W_ temp(method.c_str());
				m_request_method = (LPCWSTR)temp;
				return TRUE;
			}
			return FALSE;
		}
		...
		BOOL Send(LPVOID lpPostData = NULL, DWORD dwsize = 0)
		{
			...
			std::wstring wstrHeaders((LPCWSTR)tempHeaders);
		...
		retry:
			...
			textconv_helper::W2A_ temp(result.c_str());
			return std::string((LPCSTR)temp);
		}
		...
		{
			textconv_helper::A2W_ tempUser(username.c_str());
			this->m_basic_auth_user = (LPCWSTR)tempUser;
			textconv_helper::A2W_ tempPass(password.c_str());
			this->m_basic_auth_pass = (LPCWSTR)tempPass;
		}

		void SetProxy(const std::string& proxy, const std::string& proxyName = "", const std::string& proxyPass = "")
		{
			textconv_helper::A2W_ tempProxy(proxy.c_str());
			this->m_proxy_info = (LPCWSTR)tempProxy;
			textconv_helper::A2W_ tempUser(proxyName.c_str());
			this->m_proxy_user = (LPCWSTR)tempUser;
			textconv_helper::A2W_ tempPass(proxyPass.c_str());
			this->m_proxy_pass = (LPCWSTR)tempPass;
		}
	};
}
