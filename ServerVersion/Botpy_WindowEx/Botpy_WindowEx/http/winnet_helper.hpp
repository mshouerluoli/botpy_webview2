...
namespace winnet
{
	...
	public:
		...
		BOOL Send(LPVOID lpPostData = NULL, DWORD dwsize = 0)
		{
			...
			{
				if (!InternetSetOptionA(m_hConnect, INTERNET_OPTION_USERNAME, (LPVOID)m_basic_auth_user.c_str(), (DWORD)(m_basic_auth_user.size() + 1)))
				{
					printf("Error %u in InternetSetOptionA. INTERNET_OPTION_USERNAME \n", GetLastError());
					return FALSE;
				}
				if (!InternetSetOptionA(m_hConnect, INTERNET_OPTION_PASSWORD, (LPVOID)m_basic_auth_pass.c_str(), (DWORD)(m_basic_auth_pass.size() + 1)))
				{
			...
			if (!m_proxy_info.empty())
			{
				...
				{
					if (!InternetSetOptionA(m_hConnect, INTERNET_OPTION_PROXY_USERNAME, (LPVOID)m_proxy_user.c_str(), (DWORD)(m_proxy_user.size() + 1)))
					{
						printf("Error %u in InternetSetOptionA. INTERNET_OPTION_PROXY_USERNAME \n", GetLastError());
						return FALSE;
					}
					if (!InternetSetOptionA(m_hConnect, INTERNET_OPTION_PROXY_PASSWORD, (LPVOID)m_proxy_pass.c_str(), (DWORD)(m_proxy_pass.size() + 1)))
					{
		...
		std::string GetResponseHeaderValue(const std::string& name)
		{
			...
			DWORD			dwSize			= (DWORD)(name.length() + 1);
