        ...
        if (m_websocket_connected && m_websocket) {
            if (m_heartbeat_interval > 0) {
                ...
                if (!send_result) {
                    ...
                    if (on_status) on_status("connecting", (char*)u8"正在重连...");
                    stop();
            ...
            if (m_running) {
                ...
                if (on_status) on_status("connecting", (char*)u8"连接失败，正在重连...");
                stop();
                if (on_restart) on_restart();
                return;
            }
...
// FNV-1a 64-bit hash — no external deps, fixed-size output, fast enough for
